...
class AuditView {
  ...
  async load(filters = {}) {
    ...
    if (this.filters.governance_only) params.append('governance_only', 'true');
  ...
  renderControls() {
    ...
    entitySelect.style.cssText = 'padding: 8px 10px; border-radius: 6px; border: 1px solid var(--border, #ddd); background: var(--surface, #fff);';
    this.populateSelect(entitySelect, ['workflow', 'task'], this.filters.entity_type, 'All entities');
    entitySelect.addEventListener('change', () => this.load({ entity_type: entitySelect.value, offset: 0 }));
    ...
    const governanceWrap = document.createElement('label');
    governanceWrap.style.cssText = 'display: inline-flex; align-items: center; gap: 6px; font-size: 0.85rem; color: var(--muted, #666); padding: 6px 8px; border: 1px solid var(--border, #ddd); border-radius: 6px; background: var(--surface, #fff);';
    const governanceToggle = document.createElement('input');
    governanceToggle.type = 'checkbox';
    governanceToggle.checked = Boolean(this.filters.governance_only);
    governanceToggle.addEventListener('change', () => {
      this.load({ governance_only: governanceToggle.checked, offset: 0 });
    });
    const governanceText = document.createElement('span');
    governanceText.textContent = 'Operator actions only';
    governanceWrap.append(governanceToggle, governanceText);
  ...
  renderTable(records) {
    ...
    thead.innerHTML = `
      <tr style="background: var(--surface-2, #f0f0f0); text-align: left;">
        ...
        <th style="padding: 10px; border-bottom: 1px solid var(--border, #ddd);">Entity</th>
    ...
    records.forEach(record => {
      ...
      tr.innerHTML = `
        ...
        <td style="padding: 10px;"><span style="display:inline-block; padding:2px 8px; border-radius:999px; background:${record.entity_type === 'workflow' ? 'rgba(99, 102, 241, 0.12)' : 'rgba(148, 163, 184, 0.16)'};">${this.escapeHtml(record.entity_type || 'task')}</span></td>

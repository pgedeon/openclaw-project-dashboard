 ...
 * 
 * Version: v4 (2026-03-13) - Force cache invalidation
 */

const CACHE_NAME = 'openclaw-dashboard-v4';
const RUNTIME_CACHE = 'openclaw-dashboard-runtime-v4';
const API_CACHE = 'openclaw-dashboard-api-v4';
const DISABLE_ON_LOCAL_DEV = self.location.hostname === 'localhost' || self.location.hostname === '127.0.0.1';
...
self.addEventListener('install', (event) => {
  if (DISABLE_ON_LOCAL_DEV) {
    console.log('[ServiceWorker] Local dev detected, skipping install cache setup');
...
self.addEventListener('activate', (event) => {
  if (DISABLE_ON_LOCAL_DEV) {
    console.log('[ServiceWorker] Local dev detected, unregistering service worker');
    event.waitUntil(
      (async () => {
        const cacheNames = await caches.keys();
        await Promise.all(
          cacheNames
            .filter((name) => name.startsWith('openclaw-dashboard-'))
...
self.addEventListener('message', (event) => {
  ...
  // Handle SELF_DESTRUCT for aggressive cache clearing
  if (event.data && event.data.type === 'SELF_DESTRUCT') {
    console.log('[ServiceWorker] Self-destruct requested, clearing all caches and unregistering');
    event.waitUntil(
      (async () => {
        const cacheNames = await caches.keys();
        await Promise.all(

...
By default the script writes logs under `$OPENCLAW_WORKSPACE/logs` and stores the PID at `$OPENCLAW_WORKSPACE/.dashboard.pid`. Override with `DASHBOARD_LOG_DIR`, `DASHBOARD_HEALTH_LOG_FILE`, `DASHBOARD_SERVER_LOG_FILE`, or `DASHBOARD_PID_FILE` if needed.

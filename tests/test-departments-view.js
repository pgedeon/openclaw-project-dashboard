...
function run() {
  ...
  assert.ok(
    js.includes("view === 'departments'"),
    'dashboard integration should route the departments view'
  );
  assert.ok(
    js.includes('async function renderDepartmentOpsView(state)'),
    'dashboard integration should define the department operations renderer'
  );
  assert.ok(
    js.includes("fetch('/api/org/departments'"),
    'department operations renderer should load the department catalog'
  );
  assert.ok(
    js.includes("fetch(`/api/org/departments/${encodeURIComponent(selectedDepartmentId)}/operating-view`"),
